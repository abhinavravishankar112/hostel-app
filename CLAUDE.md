...
**Compatibility scoring lives in [server/utils/compatibility.js](server/utils/compatibility.js).** Weights sum to 100 so a score reads as a percentage. A dimension is scored only when *both* profiles have the field; the remaining weights are renormalized, so a sparse profile lowers `confidence` rather than the score, and a pair with nothing in common yields `score: null` (not 0) — the UI must branch on null rather than rendering 0%. `GET /api/users/hostel` attaches a `compatibility` summary to every member except the caller and strips `breakdown` to keep the list small; `GET /api/users/:id` includes the full `breakdown`. Both endpoints use `.lean()` because they spread computed fields onto the documents. Adding a scored field means updating `WEIGHTS`, `LABELS`, and a dimension scorer together.
